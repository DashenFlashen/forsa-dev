...
from forsa_dev.ports import allocate_port, allocate_ports
from forsa_dev.state import Environment, save_state


def _env(name: str, port: int, state_dir: Path, ttyd_port: int | None = None) -> Environment:
    return Environment(
        name=name,
        user="anders",
        branch=name,
...
def test_allocate_port_skips_used_ttyd_port(tmp_path):
    ...
    save_state(_env("x", port=3001, state_dir=tmp_path, ttyd_port=3000), tmp_path)
    with allocate_port(tmp_path, 3000, 3099) as port:
